...
- Arrival block selectors support exact blocks, tags, and `{"any":true}`. Exact placement/support combinations outrank tag matches, which outrank generic `any` rules. Plain Dirt is a corporeal resident, while Grass Block, Coarse Dirt, Podzol, Mycelium, Sand, and Red Sand are authored ground cardinals gated by the samurai boots. Implemented ore families use their collected drops plus ordinary torch placement on `minecraft:base_stone_overworld`, producing one canonical identity per ore family; Copper remains deferred until its Moe texture and spawn support exist.
