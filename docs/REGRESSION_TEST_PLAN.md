...
This plan turns the architecture, behavior contract, and technical debt notes into a concrete test backlog. It does not implement tests. The intent is to stop hotfix churn by capturing current behavior before remaining bug fixes and before future Minecraft/NeoForge version moves.

Timing labels:

- `Before bug fixes`: write or manually baseline this before changing known risky behavior.
- `Current NeoForge baseline`: required before changing active NeoForge 1.21.4 behavior or moving Minecraft/NeoForge versions again.
...
- Why it matters for NeoForge/current-version stability: touches item use, block tags, entity creation, synced data, DB insertion, and world mutation. This is the core smoke test for the mod.
- Timing: Current NeoForge baseline.
- Likely classes/files: `src/main/java/block_party/items/CustomSpawnEggItem.java`, `src/main/java/block_party/entities/Moe.java`, `src/main/java/block_party/registry/CustomEntities.java`, `src/main/java/block_party/db/records/NPC.java`, `src/main/java/block_party/db/BlockPartyDB.java`, `src/main/resources/data/block_party/tags/block/spawns_moes.json`.
...
- Why it matters for NeoForge/current-version stability: tag lookup and item interaction APIs are version-sensitive; accidental broad spawning would break the core block-person contract.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: block state IDs, block entity APIs, and NBT access often change across Minecraft versions.
- Timing: Current NeoForge baseline.
- Likely classes/files: `CustomSpawnEggItem.java`, `Moe.java`, `MoeInHiding.java`, block entity classes under `src/main/java/block_party/blocks/entity/`.
...
- Why it matters for NeoForge/current-version stability: tag paths and registries are version-sensitive; traits drive rendering, sounds, names, and later behavior.
- Timing: Current NeoForge baseline.
- Likely classes/files: `Moe.java`, `CustomTags.java`, trait JSON under `src/main/resources/data/block_party/tags/block/moe/`.
...
- Why it matters for NeoForge/current-version stability: combines `SavedData`, DB records, owner UUID, and resource-loaded names.
- Timing: Current NeoForge baseline.
- Likely classes/files: `Moe.java`, `Recordable.java`, `Gender.java`, `Names.java`, `BlockPartyDB.java`, `NPC.java`.
...
- Why it matters for NeoForge/current-version stability: verifies resource reload, scene filters/actions, interaction hooks, and server-side dialogue state before UI packets are involved.
- Timing: Current NeoForge baseline.
- Likely classes/files: `Moe.java`, `SceneManager.java`, `Scenes.java`, `SendDialogue.java`, `src/main/resources/data/block_party/scenes/test_dialogue.json`.

### `dialogue_response_packet_advances_scene`

- System under test: dialogue packets.
- Behavior protected: `DialogueRespondPayload` finds the right NPC record/server entity and sets the selected `Response`, allowing the scene to advance.
- Why it matters for NeoForge/current-version stability: networking APIs are high-risk; this protects the server-side contract behind `DialogueScreen` buttons.
- Timing: Current NeoForge baseline.
- Likely classes/files: `DialogueRespondPayload.java`, `NpcDetailPayload.java`, `Moe.java`, `SendDialogue.java`, `SendResponse.java`, `Response.java`.
...
- Why it matters for NeoForge/current-version stability: covers scene action execution, entity removal, block placement, saved data, and DB update.
- Timing: Before bug fixes.
- Likely classes/files: `Moe.java`, `Hide.java`, `MoeInHiding.java`, `HidingSpots.java`, `NPC.java`, `test_hide.json`.
...
- Why it matters for NeoForge/current-version stability: timing, entity tick, saved data, and block mutation APIs are all version-sensitive.
- Timing: Before bug fixes.
...
- Why it matters for NeoForge/current-version stability: event class names and ordering will change; missing one event creates hard-to-debug hidden-Moe loss.
- Timing: Current NeoForge baseline.
- Likely classes/files: `HidingSpots.java`, `MoeInHiding.java`, NeoForge event hook usage, `BlockEvent`, `PlayerInteractEvent`, `PistonEvent`, `EntityJoinLevelEvent`.
...
- Why it matters for NeoForge/current-version stability: persistence APIs can change, and the fixed behavior should not regress during future version moves.
- Timing: Current NeoForge baseline.
- Likely classes/files: `MoeInHiding.java`, `HideUntil.java`, `HidingSpots.java`, `Moe.java`, `NPC.java`.
...
- Why it matters for NeoForge/current-version stability: protects `SynchedEntityData`, entity NBT, SQLite rows, and world saved data.
- Timing: Current NeoForge baseline.
- Likely classes/files: `Moe.java`, `MoeInHiding.java`, `BlockPartyDB.java`, `NPC.java`, `Column.java`, `Row.java`.
...
- Why it matters for NeoForge/current-version stability: Yearbook and Cell Phone are useless if saved-data changes lose this list.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: protects the packet-to-DB-to-screen data path without depending on client rendering.
- Timing: Current NeoForge baseline.
- Likely classes/files: `YearbookItem.java`, `ControllerOpenPayload.java`, `NpcDetailPayload.java`, `NPC.java`, `ControllerScreen.java`.
...
- Why it matters for NeoForge/current-version stability: owner lookups and player references can change; this prevents accidental privacy regressions.
- Timing: Before release.
- Likely classes/files: `YearbookItem.java`, `Moe.java`, `BlockPartyDB.java`, `NPC.java`.
...
- Why it matters for NeoForge/current-version stability: combines packets, DB lookup, forced chunks, teleport APIs, dimension behavior, and entity synced data.
- Timing: Current NeoForge baseline.
- Likely classes/files: `CellPhoneItem.java`, `ControllerOpenPayload.java`, `NpcCallRequestPayload.java`, `NpcCallPayload.java`, `CellPhone.java`, `MoeSpawner.java`, `NPC.java`, `Moe.java`.
...
- Why it matters for NeoForge/current-version stability: prevents silent changes from "fails gracefully" to crash, or vice versa, during API changes.
- Timing: Current NeoForge baseline.
- Likely classes/files: `NpcCallRequestPayload.java`, `NpcCallPayload.java`, `NPC.java`, `CellPhone.java`, `MoeSpawner.java`.

### `shrine_list_sync_matches_owner_and_dimension_rule`

- System under test: shrine/location sync.
- Behavior protected: `ShrineListPayload` includes the same shrine positions under the current owner/dimension rules.
- Why it matters for NeoForge/current-version stability: player login events, DB queries, and client-side location data are API-sensitive.
- Timing: Before release.
- Likely classes/files: `ShrineListPayload.java`, `Shrine.java`, `BlockPartyDB.java`, `ShrineLocation.java`, `ShrineTabletBlockEntity.java`.
...
- Why it matters for NeoForge/current-version stability: reload listener APIs are one of the highest-risk port areas.
- Timing: Current NeoForge baseline.
...
- Behavior protected: verifies active Moe attacks route through vanilla combat delegation without recursive stack overflow.
- Why it matters for NeoForge/current-version stability: entity combat methods are API-sensitive, and the resolved super-call behavior should be preserved.
- Timing: Current NeoForge baseline.
- Likely classes/files: `Moe.java`, `Moe.java`, `Moe.java`, `CustomEntities.java`.
...
- Why it matters for NeoForge/current-version stability: supports the hide/reveal bug fix and prevents future data-driven hide regressions.
- Timing: Before bug fixes.
...
- Why it matters for NeoForge/current-version stability: if scene interruption changes accidentally, dialogue/hide behavior will feel random.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: packet payloads and scene JSON use response enum names.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: DB rows, NBT, scene filters, and data-pack tags encode these names.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: personality generation is scattered and easy to perturb.
- Timing: Before release.
- Likely classes/files: `BloodType.java`, `Moe.java`, `CustomSpawnEggItem.java`.
...
- Why it matters for NeoForge/current-version stability: DB columns, home positions, locations, and hidden spots rely on dimension-aware positions.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: future dialogue/chore/prank state depends on these containers.
- Timing: Before release.
...
- Why it matters for NeoForge/current-version stability: `DialogueOpenPayload` sends this over the network; codec changes must not alter payload semantics.
- Timing: Current NeoForge baseline.
- Likely classes/files: `Dialogue.java`, `Speaker.java`, `DialogueOpenPayload.java`, `Response.java`.
...
- Why it matters for NeoForge/current-version stability: protects `NpcDetailPayload` and `DialogueOpenPayload` payloads before networking changes.
- Timing: Current NeoForge baseline.
- Likely classes/files: `NPC.java`, `Row.java`, `Column.java`, `NpcDetailPayload.java`, `DialogueOpenPayload.java`.

### `packet_encode_decode_round_trip_dialogue_response`

- System under test: dialogue packet payload.
- Behavior protected: `DialogueRespondPayload` preserves NPC ID and selected `Response`.
- Why it matters for NeoForge/current-version stability: payload codecs will change; response buttons must still select the same choice.
- Timing: Current NeoForge baseline.
- Likely classes/files: `DialogueRespondPayload.java`, `NpcDetailPayload.java`, `Response.java`, payload records under `src/main/java/block_party/network/payload/`.

### `packet_encode_decode_round_trip_controller_packets`

- System under test: Yearbook/Cell Phone packet payloads.
- Behavior protected: `ControllerOpenPayload`, `NpcDetailPayload`, `NpcCallRequestPayload`, and `NpcCallPayload` preserve NPC IDs, selected ID, hand, and NBT payloads.
- Why it matters for NeoForge/current-version stability: protects controller screen opening and NPC list navigation.
- Timing: Current NeoForge baseline.
- Likely classes/files: `ControllerOpenPayload.java`, `NpcDetailPayload.java`, `NpcCallRequestPayload.java`, `NpcCallPayload.java`.
...
- Why it matters for NeoForge/current-version stability: dirty tracking drives SQL update generation; changing it blindly can cause DB churn or missed saves.
- Timing: Before bug fixes.
...
- Why it matters for NeoForge/current-version stability: this is a known dangerous bug area and can cause hotfix churn after persistence APIs change.
- Timing: Before bug fixes.
...
- Why it matters for NeoForge/current-version stability: position data backs home locations, NPC locations, shrine sorting, and phone lookup.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: data-pack compatibility can silently break if resource IDs are normalized differently.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: texture lookup is already suspicious; this isolates the intended pattern logic before fixing outer-map keying.
- Timing: Before bug fixes.
...
- Why it matters for NeoForge/current-version stability: prevents an experimental class from becoming a hidden behavior change.
- Timing: Before release.
...
- Why it matters for NeoForge/current-version stability: this is the smallest full gameplay loop and catches integration failures automated tests may miss.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: rendering/model layer APIs are high-risk and texture lookup has known debt.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: prevents confusing future texture regressions with pre-existing lookup bugs.
- Timing: Before bug fixes.
...
- Why it matters for NeoForge/current-version stability: GUI APIs, button APIs, sound APIs, and packet APIs are version-sensitive.
- Timing: Current NeoForge baseline.
- Likely classes/files: `DialogueScreen.java`, `RespondIconButton.java`, `RespondTextButton.java`, `DialogueOpenPayload.java`, `DialogueRespondPayload.java`, `Dialogue.java`.
...
- Why it matters for NeoForge/current-version stability: protects client screen rendering plus packet/DB payload integration.
- Timing: Current NeoForge baseline.
- Likely classes/files: `YearbookItem.java`, `YearbookScreen.java`, `ControllerOpenPayload.java`, `NpcDetailPayload.java`, `NPC.java`.
...
- Why it matters for NeoForge/current-version stability: touches UI, networking, chunk lookup/ticket handling, teleport APIs, and synced follow state.
- Timing: Current NeoForge baseline.
- Likely classes/files: `CellPhoneItem.java`, `CellPhoneScreen.java`, `ControllerOpenPayload.java`, `NpcCallRequestPayload.java`, `NpcCallPayload.java`, `CellPhone.java`, `MoeSpawner.java`, `Moe.java`.
...
- Why it matters for NeoForge/current-version stability: sound event registration, resource reload, and UI sound APIs are version-sensitive.
- Timing: Before release.
...
- Why it matters for NeoForge/current-version stability: reload listener APIs and resource IDs are major port risks.
- Timing: Current NeoForge baseline.
...
- Why it matters for NeoForge/current-version stability: future version moves must not lose companion identity or old worlds.
- Timing: Current NeoForge baseline.
- Likely classes/files: `BlockPartyDB.java`, `HidingSpots.java`, `NPC.java`, `Moe.java`, `MoeInHiding.java`, `AbstractDataBlockEntity.java`, `Shrine.java`, `Garden.java`, `Location.java`.
...
- Why it matters for NeoForge/current-version stability: location persistence is partially implemented and easy to regress silently.
- Timing: Before release.
- Likely classes/files: `AbstractDataBlockEntity.java`, `LocativeBlockEntity.java`, `GardenLanternBlockEntity.java`, `ShrineTabletBlockEntity.java`, `ShrineListPayload.java`, `ShrineLocation.java`, `JapanRenderer.java`.
...
- Why it matters for NeoForge/current-version stability: render layers are registered through `BlockPartyClientEvents`, so visual screenshots should confirm the restored modern client path.
- Timing: Before bug fixes.
- Likely classes/files: `BlockPartyClientEvents.java`, `CustomBlocks.java`, block model/state resources under `src/main/resources/assets/block_party/`, client renderer setup.
...
- Why it matters for NeoForge/current-version stability: prevents planned features from being accidentally treated as current-version regressions.
- Timing: Before release.
- Likely classes/files: `Moe.java`, `NPC.java`, `CellPhone.java`, `MoeSpawner.java`, README behavior expectations.
...
6. Move Minecraft/NeoForge versions again only after the current-baseline GameTests and golden-world checks are usable.
