...
- `src/neoforgeSpike/java/block_party/gametest/BlockEntityGameTests.java`: server-side block entity parity tests for legacy block entity IDs, block placement shell creation, data block NBT round-trip, SQLite table creation, locative row fields, row deletion, and shrine list filtering.
...
| Shrine/Garden/Location/Sapling SQLite tables | `src/neoforgeSpike/java/block_party/db/BlockPartyDB.java`, `src/neoforgeSpike/java/block_party/blocks/entity/**` | Reintroduced as minimal server-side tables `Shrines`, `GardenLanterns`, `Locations`, and `SakuraSaplings` with Forge baseline column names for row identity, dimension/position, and owner UUID. `Locations` also stores `RequiredCondition` and `Priority`. Table creation, data-block claim/update/delete, locative fields, and owner/dimension shrine list queries are GameTested. |
...
| --- | --- | --- |
| `Moe` entity shell | `src/neoforgeSpike/java/block_party/entities/Moe.java` | Reintroduced as a plain NeoForge-compatible `Entity` with synced `DatabaseID`, `OwnerUUID`, `BlockState`, `Following`, `GivenName`, and `Gender`, plus local copied block-entity persistent data. NBT uses legacy `DatabaseID`, `BlockState`, `Following`, and `TileEntity` keys plus explicit `OwnerUUID`, `GivenName`, and `Gender` spike keys. |
...
- Slice 1.4 does not execute Forge shrine tablet side effects yet. The frozen Forge shrine tablet creates lightning, spawns a Moe below the tablet, plays sound, and broadcasts `SShrineList` after row changes. The NeoForge spike currently restores only server-side block entity IDs, NBT, SQLite rows, and shrine list query behavior; visual/audio/entity side effects and shrine packets are deferred to later entity/network/client slices.
