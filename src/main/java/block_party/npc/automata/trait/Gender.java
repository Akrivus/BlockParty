...
public enum Gender implements ITrait<Gender> {
    ...
    Gender(String nominative, String accusative, String possessive, String reflexive, String honorific, String... names) {
    ...
    protected List<String> getUnclaimedNames(Level level) {
        return Arrays.asList(this.names).stream().filter((name) -> !BlockPartyDB.get(level).names.contains(name)).collect(Collectors.toList());
    }
}
