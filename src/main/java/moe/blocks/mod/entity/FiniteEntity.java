...
import moe.blocks.mod.entity.dere.AbstractDere;
import moe.blocks.mod.entity.dere.Himedere;
import moe.blocks.mod.entity.emotion.AbstractEmotion;
import moe.blocks.mod.entity.emotion.NormalEmotion;
import moe.blocks.mod.entity.goal.*;
import moe.blocks.mod.entity.goal.engage.ShareGoals;
import moe.blocks.mod.entity.goal.engage.SocializeGoal;
import moe.blocks.mod.entity.goal.target.*;
import moe.blocks.mod.entity.util.*;
import moe.blocks.mod.entity.state.FoodState;
import moe.blocks.mod.entity.dating.DatingState;
import moe.blocks.mod.entity.state.StressState;
...
public class FiniteEntity extends CreatureEntity {
    public static final DataParameter<Integer> ANIMATION = EntityDataManager.createKey(FiniteEntity.class, DataSerializers.VARINT);
    public static final DataParameter<Integer> BLOOD_TYPE = EntityDataManager.createKey(FiniteEntity.class, DataSerializers.VARINT);
    public static final DataParameter<Integer> DERE = EntityDataManager.createKey(FiniteEntity.class, DataSerializers.VARINT);
    public static final DataParameter<Integer> EMOTION = EntityDataManager.createKey(FiniteEntity.class, DataSerializers.VARINT);
    public static final DataParameter<Boolean> EYEPATCH = EntityDataManager.createKey(FiniteEntity.class, DataSerializers.BOOLEAN);
    public static final DataParameter<Boolean> SITTING = EntityDataManager.createKey(FiniteEntity.class, DataSerializers.BOOLEAN);
    public AttackGoals.Melee defaultAttackStyle;
    protected Animation animation = new Animation();
    protected FoodState foodState = new FoodState();
    protected DatingState datingState = new DatingState();
    protected StressState stressState = new StressState();
    ...
    protected FiniteEntity(EntityType<? extends CreatureEntity> type, World world) {
        super(type, world);
        ...
        this.foodState.start(this);
        this.datingState.start(this);
        this.stressState.start(this);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new AvoidGoal(this));
        this.goalSelector.addGoal(3, new AttackGoals.Ranged(this));
        this.goalSelector.addGoal(3, new AttackGoals.Melee(this));
        this.goalSelector.addGoal(4, new ShareGoals.Student(this));
        this.goalSelector.addGoal(4, new ShareGoals.Player(this));
        this.goalSelector.addGoal(5, new GrabGoal(this));
        this.goalSelector.addGoal(6, new FollowGoal(this));
        this.goalSelector.addGoal(7, new MineGoal(this));
    ...
    protected boolean shouldExchangeEquipment(ItemStack candidate, ItemStack existing) {
        ...
        if (EnchantmentHelper.hasBindingCurse(existing)) {
            return false;
        } else if (slot == EquipmentSlotType.OFFHAND && this.foodState.canConsume(candidate)) {
            return true;
    ...
    public void setHeldItem(Hand hand, ItemStack stack) {
        this.entityDropItem(this.getHeldItem(hand));
    ...
    public boolean canPickUpItem(ItemStack stack) {
        if (stack.getItem().isIn(MoeTags.EQUIPPABLES) || this.foodState.canConsume(stack)) {
    ...
    public boolean canDoChores() {
        return this.getDatingState().iterate(relationship -> relationship.canDoChoresFor());
    }
    ...
    public DatingState getDatingState() {
    ...
    public boolean isCompatible(FiniteEntity entity) {
    ...
    public void swingArm(Hand hand) {
        this.foodState.addExhaustion(0.1F);
        super.swingArm(hand);
    }

    @Override
    public void jump() {
        this.foodState.addExhaustion(this.isSprinting() ? 0.2F : 0.05F);
        super.jump();
    }
